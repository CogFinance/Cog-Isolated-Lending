...
def mint_tokens_for_testing(project: Project, account):
    ...
    token_contract.mint(account,100 * 10 ** 18, sender=token_owner)
...
def test_curve_eth_pair(cog_pair,project, weth, crv, oracle, account):
    ...
    crv.approve(cog_pair.address, 100 * 10 ** 18, sender=account)
    weth.approve(cog_pair.address, 100 * 10 ** 18, sender=account)

    # 5 CRV collateral
    cog_pair.add_collateral(account,5 * 10 ** 18, sender=account)
    cog_pair.deposit(5 * 10 ** 18, sender=account)

    cog_pair.get_exchange_rate(sender=account)
    print(cog_pair.exchange_rate())
    BASE_AMT = 1505546750000000

    with ape.reverts():
        cog_pair.borrow(int(BASE_AMT * 0.8), sender=account)

    cog_pair.borrow(int(BASE_AMT * 0.7), sender=account)

...
interface PoolSharksRangePool:
    # secondsAgos: uint32[5], tickSecondsAccum, secondsPerLiquidityCumulativeX128, averagePrice, averageLiquidity, averageTick
    def sample(secondsAgos: uint32[5]) -> (int56[5], uint160[5], uint160, uint128, int32): view
...
def fetch_poolsharks_twap(oracle: address) -> uint256:
    ...
    seconds_per_liquidity_cumulative_x128s: uint160[5] = [0, 0, 0, 0, 0]

    average_price: uint160 = 0
    average_liquidity: uint128 = 0
    average_tick: int32 = 0

    tick_seconds_accum, seconds_per_liquidity_cumulative_x128s, average_price, average_liquidity, average_tick  = PoolSharksRangePool(oracle).sample(seconds_ago)

    return convert(average_price, uint256)
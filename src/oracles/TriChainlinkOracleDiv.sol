...
contract TriChainlinkOracleDiv {
    address immutable multiply;
    address immutable divide2;
    address immutable divide;
    uint256 immutable decimals;

    constructor(address _mul, address div2 ,address _div, uint256 _dec) {
        multiply = _mul;
    ...
    function _get() internal view returns (uint256) {
        ...
        if (divide != address(0)) {
            ...
            int256 div2Price = IAggregatorV2(divide2).latestAnswer();
            require(div2Price != 0, "Invalid div2Price");
            price = price / (uint256(divPrice * div2Price) / 10 ** 18);
        }

...
contract AmbientOracle is IOracle {
    ...
    address public immutable priceToken;
    ...
    constructor(address _lens, address tokenAddress, address tokenA, address tokenB) {
    
      (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);

      lens = AmbientQuery(_lens);
    ...
    function calculatePrice() internal view returns (uint256) {
        ...
        // fullPrice is currently in Q64.64
    ...
    function name() external pure returns (string memory) {
        return "Ambient LP Token Oracle";
    }

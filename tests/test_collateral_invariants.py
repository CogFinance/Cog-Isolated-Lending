...
def test_add_collateral(cog_pair, amount, accounts, collateral):
    ...
    events = cog_pair.add_collateral(account, amount, sender=account).events[1]
    after_balance_0 = cog_pair.user_collateral_share(account)

    # The function emits a `AddCollateral event` with the arguments msg.sender, to, and share.
    assert events.to == account
    assert events.amount == amount
    assert events.user_collateral_share == after_balance_0
...
def test_remove_collateral(cog_pair, amount, accounts, collateral):
    ...
    events = cog_pair.remove_collateral(account, amount, sender=account).events[1]

    after_balance_0 = cog_pair.user_collateral_share(account)

    # The function emits a `RemoveCollateral event` with the arguments to, amount, and share.
    assert events.to == account
    assert events.amount == amount
    assert events.user_collateral_share == after_balance_0
